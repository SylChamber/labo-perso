...
> L'emplacement par défaut de la configuration de CoreDNS est le dossier où se trouve l'exécutable de CoreDNS. L'option `-conf` ajoutée à CoreDNS permet de configurer l'emplacement du fichier de configuration `Corefile`. Le point de montage `/etc/coredns/Corefile` monté comme volume. L'option `-d` de `podlet` ajoute une description à l'unité systemd.

```shell
podlet -d "CoreDNS - DNS and Service Discovery" \
    podman run --name coredns \
    -d -p 53:53 -p 53:53/udp \
    -v /var/lib/coredns/etc/Corefile:/etc/coredns/Corefile:ro \
    -v /var/lib/coredns/etc/domicile.internal.db:/etc/coredns/domicile.internal.db:ro \
...
Volume=/var/lib/coredns/etc/Corefile:/etc/coredns/Corefile:ro
Volume=/var/lib/coredns/etc/domicile.internal.db:/etc/coredns/domicile.internal.db:ro
```
...
PublishPort=53:53
PublishPort=53:53/udp
Volume=/var/lib/coredns/etc:/etc/coredns/Corefile:ro
...
NotifyAccess=all
SyslogIdentifier=%N
ExecStart=/usr/bin/podman run --name coredns --replace --rm --cgroups=split --sdnotify=conmon -d -v /var/lib/coredns/etc:/etc/coredns/Corefile:ro --publish 53:53 --publish 53:53/udp coredns/coredns:1.12.4
```
