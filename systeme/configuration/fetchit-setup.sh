...
FETCHIT_CONFIG_DIR=/etc/containers/fetchit
...
Restart=always
TimeoutStopSec=65
ExecStartPre=/usr/bin/mkdir -p /etc/containers/fetchit
ExecStartPre=/bin/rm -f %t/%n.ctr-id
ExecStart=/usr/bin/podman run --cidfile=%t/%n.ctr-id --cgroups=no-conmon --rm --security-opt label=disable --sdnotify=conmon --replace --label io.containers.autoupdate=registry -d --name fetchit -v fetchit-volume:/opt -v /etc/containers/fetchit:/opt/mount -v /run/podman/podman.sock:/run/podman/podman.sock quay.io/fetchit/fetchit:latest
